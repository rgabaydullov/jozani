import { ROWS_MAX, SYMBOL_WIDTH } from './constants';
...
export const getSymbolsInRow = (
  ...
  const symbolsByRow = parseInt(`${el?.clientWidth / SYMBOL_WIDTH}`, 10);
...
export const getRows = (
  ...
  const linesByString = parseInt(`${comment.length / (symbolsInRow)}`, 10) + 1;
